...
void compare_small_with_brute_force(const CSCMatrix<Rational>& matrix,
                                    size_t max_diff) {
...
TEST(SmallRowsTests, CompareWithBruteForce1) {
  for (size_t i = 0; i < 50 && i < benchmark_set.size(); ++i) {
    ...
    ASSERT_NO_FATAL_FAILURE(compare_small_with_brute_force(matrix, 1));
  }
}
...
TEST(SmallRowsTests, CompareWithBruteForce2) {
  for (size_t i = 0; i < 50 && i < benchmark_set.size(); ++i) {
    ...
    auto matrix = get_archived<Rational>(benchmark_set[i]).A;
    ASSERT_NO_FATAL_FAILURE(compare_small_with_brute_force(matrix, 2));
  }
}
...
TEST(SmallRowsTests, CompareWithBruteForce3) {
  for (size_t i = 0; i < 50 && i < benchmark_set.size(); ++i) {
    ...
    auto matrix = get_archived<Rational>(benchmark_set[i]).A;
    ASSERT_NO_FATAL_FAILURE(compare_small_with_brute_force(matrix, 3));
  }
}
...
TEST(SmallRowsTests, RandomizedTest2) {
  ...
  for (size_t i = 0; i < 100'000; ++i) {
    ...
    ASSERT_NO_FATAL_FAILURE(compare_small_with_brute_force(problem.A, 2));
  }
}

TEST(SmallRowsTests, IgnoresBigRows) {
